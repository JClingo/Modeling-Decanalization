...
The phase 3 comparison (recanalization vs. canalization) was critical because it directly compared agents that did or did not undergo a decanalization phase. The agent in the recanalization condition managed mean \(h\) more effectively.  The $\Delta M$ of -42.77 indicates that agents that went through decanlization and recanalization (CDR) maintained a $h$-value that was on average 42.77 closer to setpoint than agents in the canalization only condition (CCC). The CDR agents also showed much lower setpoint deviation than the CCC agents, meaning that they maintained their $h$-values in a narrower range (smaller standard deviation over all timesteps in the phase). The CDR agents predicted more reward than CCC agents on average. Interestingly, the CDR agents experienced more loss in \(h\) than CCC agents. This increased loss was offset by greater \(h\) gain through pellet consumption, although pellet diversity did not increase, suggesting that decanalized agents located higher-payout pellets in less-traversed regions.

These results suggest that decanalization can lead to spontaneous exploration of new pathways, resulting in improved homeostatic performance. The decanalized agent modestly improved its reward prediction, which was reflected in its improved ability to stay close to the setpoint.
...
Phase 3 performance of the decanalization versus no-decanalization agent was strikingly different from what we saw with HRL. The \(h\)-mean performance difference was close to the conventional significance threshold, with the decanalized agent staying a bit closer to 0, suggesting improved performance. Most of the other results were flat across conditions, with the strong exceptions of reward prediction and \(h\) lost, where the decanalized agent saw a significantly lower reward prediction and a higher loss in \(h\). As with HRL, this could suggest that decanalization can result in pushing the agent into a less-explored region of the landscape. However, unlike HRL, RL-trained agents do not appear to benefit in the homeostatic sense discussed here. They were pushed outside their trajectories, but they did not spontaneously identify pathways that could improve their homeostatic performance by keeping their \(h\) value above zero.
...
\begin{table}
    ...
    \resizebox{\linewidth}{!}{
        \begin{tabular}{|l|
            ...
            Reward prediction    & \multicolumn{1}{|l|}{0.353}                                                              & \multicolumn{1}{|l|}{-0.204}  & \multicolumn{1}{|l|}{0.41}     \\ \hline
        ...
        \end{tabular}}
    \caption{Difference-of-differences summary comparing the effects of decanalization on HRL- and NF-trained agents across the three phases. Each entry is the decanalization effect for HRL minus the corresponding decanalization effect for NF.}
