...
For each reward function (HRL and NF), we trained multiple agents from independent random seeds. Across seeds, agents converged on qualitatively similar policies and similar end-of-training performance. We therefore focus on aggregate trial-level performance, using individual trained agents primarily for visualization and qualitative illustration.
...
Because several outcome measures are related and the present study is primarily a proof-of-principle computational analysis, the inferential statistics should be read as descriptive support for broader behavioral patterns rather than as the sole basis for the paper's claims. We therefore emphasize consistency across matched trials, effect sizes, and convergence across independently trained seeds, and we do not treat any single threshold-crossing \(p\)-value as dispositive. All stochastic elements of training and evaluation were seed-controlled so that environment generation, initial transforms, and condition matching could be reproduced from the recorded parameter settings.
...
\begin{figure}
    ...
    \caption{Evaluation design. Within each matched CDR-CCC comparison, agents were evaluated on the same seeded pellet arrangement and starting transform, with the phase-2 decanalization manipulation as the only intended difference. Across trials, the seeds varied, producing different environment instances from the same generation procedure.}
\end{figure}
...
Although we trained multiple agents from independent seeds, cross-seed variability was not the primary focus of the present analysis. A fuller treatment of between-seed differences, training dynamics, robustness to hyperparameter choices, and ablations of the composite decanalization manipulation would strengthen future work.
