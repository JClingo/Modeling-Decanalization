...
Let
\begin{align*}
\mathbf{O} = \left\{ o_1,o_2,...o_{n} \right\}  &\quad\text{be the }h \text{ values of pellets intersected by the raycast at time $t$}\\
\mathbf{T} = \left\{\tau_1,\tau_2,...,\tau_{m}\right\}  &\quad\text{be the tiles intersected by the raycast at time $t$}\\
\end{align*}
and
\begin{equation}
\hat{h}_t = h_{t+\kappa}|(a_{[t, t+\kappa]}= \varnothing)  = h_t + \sum_{o_i \in \mathbf{O}} o_{i} - \sum_{\tau_j \in \mathbf{T}} \hat{c}_{t}(\tau_j) - l(\kappa)
\end{equation}
where
\begin{align*}
...
Between each phase, we reset the value of \(h\) to a random value from $[-15,15]$. There were 50 trials in each condition. Each trial presented a different starting transform for the agent and a different distribution of pellets in the environment, although these distributions were the same for all four conditions.
...
We compared two different versions of the trained agents (HRL and NF) under two conditions: decanalization and no-decanalization. The four experimental conditions were therefore: HRL with decanalization, HRL  without decanalization, NF with decanalization, and NF without decanalization.  

\begin{figure}
    \centering
    \includegraphics[width=0.75\linewidth]{figures/modeling-process.png}
    \caption{Simulation setup: Each agent was trained over many iterations across the same series of tile arrangements. Next, they were allowed to navigate the same series of tile arrangements. Then, depending on the condition, the tiles were or were not decanalized. Last, they returned to/remained in a regular canalized tile arrangement.}
\end{figure}
...
\subsection{HRL with decanalization vs. HRL with no-decanalization}
...
\subsection{NF with decanalization vs. NF with no-decanalization}
...
\subsection{The effects of decanalization on HRL vs NF-trained agents}
\begin{table}
\tabcolsep=0.11cm
\scalebox{0.80}{
\begin{tabular}{|l|
...
Using HRL and NF as comparative reward functions and PPO for the optimization algorithm, we simulated the canalization-decanalization-recanalization process. Agents were trained to minimize the distance from the setpoint (HRL) or to return to the setpoint once they detected that they were below it (NF). To further entrench agents and simulate learning under stress and uncertainty (which is often what precipitates hypercanalized pathologies such as depression), we made it so that repeated traversal became less costly. The interaction of discounted repeated movement, passive costs, and homeostatic rewards canalized agent behaviors. Although these behaviors were locally effective at maximizing rewards and minimizing punishments, they were far from the ideal behaviors available in the world, both in terms of reward maximization and in terms of the stated ideal (optimal setpoint distance minimization). By introducing a temporary state of decanalization, agents spontaneously escaped hypercanalized areas of the world, entering an exploratory space, forming new paths. This decanalized state was not well-suited to setpoint distance minimization but led to improved performance after the initial set of parameters were restored. With this simulation, we helped demonstrate that this slow degradation of homeostatic performance over time could be mitigated by introducing decanalization.
...
There were several items that were not included in our analysis that could have been of interest. For example, we limited ourselves to just a single agent as a well-trained representative of each reward function. In reality, we could have generated many possible agents by altering the training parameters, each with their own behavioral quirks. Instead, we only compared the agents that had the best performance during training (measured in terms of rewards given, which were themselves given based on distance from setpoint). Comparison of HRL and NF agent performance on various tasks in variations of this simulated environment could be of interest. 
...
The canalization and decanalization research space often mixes behavioral descriptions of its concepts into a chaotic slurry of neurological, mathematical, and physical models. As a result, research in this space has focused on clinical efficacy and safety organized around the question: “Did the patient's condition improve?” This approach makes predicting outcomes difficult, and has posed a practical barrier to widespread medical adoption of psychedelics (and other disintegrators), despite the high efficacy and relatively low-risk profile of psychedelic-assisted psychotherapy \parencite{gukasyan_efficacy_2022}. 

Through a concrete, formalized analysis of canalization, decanalization, and recanlization with explicit parameters, we take ourselves to have contributed to the field, providing a concrete starting point that could benefit others in the space, and provide a starting point for further incremental study and progress.

We took informal descriptions of canalization and decanalization and created a simulated environment, within which agents could be trained to explore according to their reward functions. The results indicated that decanalization can indeed improve performance. However, it matters which reward function we use. Homeostatic reinforcement learning is well-suited to the task, as it is aligned with criticality, a central feature of existing hypotheses about psychedelic-induced behavioral transformation. Negative feedback is less well-suited to the task, as it is intrinsically passive and therefore ill-suited for long-term planning, being better suited for simple environments with high predictability. 

Simulating canalization and decanalization is just a first step towards understanding a key feature of human behavior—the shift from exploitative, rigid patterns to the fluid, spontaneous exploration characterized by psychedelic states, alongside other transformative stimuli \parencite{clingoDisintegrationReintegrationModelPsychological2025}. Both modes of behavior have their advantages. Certain mental health pathologies are both the cause and the result of chronic canalization (e.g., depression, PTSD, substance use disorder) and decanalization (schizophrenia, bipolar syndrome). Overcoming them is a matter of understanding their dynamics more clearly and building tools to help us take advantage of our innate ability to change.